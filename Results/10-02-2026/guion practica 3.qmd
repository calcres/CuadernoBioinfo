...
# Preparación
Antes de comenzar conectamos lo que hagamos desde la terminal a git hub utilizando el comando "git clone"+ el enlace HHTPS de la carpeta "CuadernoBioinfo". 

Para la desarga de fasp debemos indicar ondulación-barra-fastp. EL recomienda que en una carpeta llamada bin metamos los programas. 
...
Notas sobre este puntO: opción -c y ponemos nombre + barra vertical. LO que hacemos es encaminar la orden a otro sitio, y se lo lleva a un sitio donde se pueda leer. Tras barra vertical ponemos wc -l. SOn "tuberías" que permiten ir pasando los datos de un programa a otro. Wc lo que hace es contar, less es un paginador.

EL gzip con -c se utiliza para descomprimir y mostrar el archivo pero luego guardarlo como estaba. POr eso no ocupa espacio. El sin -c lo que hace es sustituir por un archivo descomprimido y guardarlo así, ocupando espacio.
...
grep: regular expression. COn grep podemos interrogar un archivo y pedirle que nos muestre solo las lineas del archivo de la expresión regular que puede ser 1 palabra. Por ejemplo: grep ">" (ponemos comillas para que lo interprete grep antes que Bash).grep "^>" permite que aparezcan solo secuencias de principio de la linea.

¿Quien es wget? Es un programa de descargas que introduce mensajes, p.e de que la descarga va bien, número de bites descargados...etc. Todo esto son mensajes que no nos interesa tener en nuestra práctica. 
...
2. Revisa el informe generado por `fastp` en el archivo `fastp.html` y redacta un
   ...
   Dupliation crate: secuencias que tienen mismos errores, exactamente misma secuencia por lo que podrían ser errores de PCR.
