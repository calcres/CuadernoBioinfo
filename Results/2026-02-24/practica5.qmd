...
En el primero vemos que no funciona. Tiene un URL que si copiamos y pegamos en el navegador sin las comillas proporciona la tabla de resultados. El navegador es lo suficientemente inteligente como para interpretar ciertos espacios entre condiciones y el espacio cerca de AND. Las " de full el navegador también las diferencia. En R, usando "curl_dowload" necesitamos utilizar URL más "cuidadas" para que R no se detenga en los espacios. 

Control + Enter me permite ejecutar las líneas en R studio. Estas órdenes aparecen en la consola.

En el tercer bloque vemos mucho más clara y de forma más práctica la información. Me permite ver fácilmente la búsqueda por si quiero editarla. 
...
curl_download(
  ...
  destfile = 'z1.txt')
```
En este último bloque estamos estableciendo los 5 requerimientos de búsqueda avanzada. Si nos fijamos en "query" le hemos pedido que se refiera a un taxón concreto y que muestre genoma completo.Vemos como curl_dowload lo guarda en formato texto. 
...
curl_download(URLencode(), destfile = 'z2.txt')
```
En este caso vemos que el objeto URL no es capaz de identificarlo. No hemos añadido una dirección válida. 
:::

Imaginemos que nos pide buscar secuencias relacionadas con la ADH. Podríamos hacer la búsqueda en ENA y al final del resultado podemos pedir que muestre esta búsqueda en un formato parecido a este. Podemos ver qué tipo de resultado era, filtros utilizados...etc. En el grupo anterior no se han encontrado "codig" de ADH en Drosophila. 

curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -d 'result=sequence&query=tax_tree(32281)%20AND%20description%3D%22ADH%22%20AND%20mol_type%3D%22genomic%20dna%22&fields=accession%2Cdescription%2Ctax_id%2Cmol_type&format=tsv' "https://www.ebi.ac.uk/ena/portal/api/search"

curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -d 'result=sequence&query=tax_tree(32281)%20AND%20description%3D%22ADH%22%20AND%20mol_type%3D%22genomic%20dna%22&fields=accession%2Cdescription%2Cscientific_name%2Cbase_count%2Clast_updated%2Ctax_id%2Cmol_type&format=tsv' "https://www.ebi.ac.uk/ena/portal/api/search"

```{r}
portal <- 'https://www.ebi.ac.uk/ena/portal/api/search?'
result <- 'result=sequence&'
query  <- 'query=tax_tree(7215) AND description="ADH" AND mol_type="genomic dna"&'
...
```{r}
adh <- read.table('z3.txt',sep='\t')
```

Las columnas donde encuentra un apóstrofe lo toma como abrir comillas que solo cerrará hasta encontrar otras. Para solucionarlo:
```{r}
adh <- read.table('z3.txt',sep='\t', quote="")
```

```{r}
adh <- read.table('z3.txt',sep='\t',header=TRUE)
```
...
Los siguientes comandos son muy útiles para contestar estas preguntas:
dim(adh): me da la extensión
head(adh): me muestra los 6 primeros valores 
names(adh): me ayudarán a contestar las preguntas del guion

1. ¿Cuántas secuencias hay con el término "alcohol dehydrogenase" en su descripción?
2. ¿Qué longitudes tienen? 
3. ¿De cuántas especies diferentes son estas secuencias? Utilizo el scientific_name. ¿Como saber cuántos valores diferentes hay en esta columna? Con unique(adh$scientific_name). Con length(unique(adh$scientific_name)) puedo conocer el valor máximo del vector. 
4. ¿Cuándo se publicaron las más recientes?Nos informará la columna first_public. Para conocer su formato usamos head(adh$fist_public). Para ver la más reciente podemos usar max y con tail(sort(adh$fist_public))) los 6 últimos valores. Sort me permite ordenar los valores de menor a mayor de forma automática.

Accedemos una columna concreta con el $. adh$base_count me da las longitudes. 
:::

# Trabajar con los resultados
(Este ejercicio 3 está redactado como si el URL no hubiese sido modificado). 
...
Entendemos que busca una modificación de la tabla que indique como cambia la longitud de la tabla según las fechas por orden. 
names(platelmintos)
...
platcumLen <- cumsum(plat$base_count)
plot(plat$last_updated, platcumLen, type='l')
```
:::
